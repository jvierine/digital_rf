...
$Id: digital_rf_archive.py 1016 2016-02-29 13:53:26Z brideout $
"""
...
def archive_subdirectory_local_local(args):
    ...
    source, channel, subdir, dt, nextDT, is_metadata, channel_dest, startDT, endDT, gzip, verbose = args
    
    # determine if this channel is compressed
    if is_metadata:
        archive_files = glob.glob(os.path.join(source, channel, subdir, _metadata_file_glob))
    else:
    ...
    if dt > endDT:
        if verbose:
            print('skipping %s because after period %s - %s' % (subdir, str(startDT), 
                                                                str(endDT)))
        return
    ...
    if dt < startDT and nextDT != None:
        if nextDT < startDT:
            if verbose:
                print('skipping %s because entirely before period %s - %s' % (subdir, str(startDT), 
                                                                              str(endDT)))
            return
    ...
    if startDT < dt and nextDT != None:
        if nextDT < endDT:
            if (is_compressed or gzip == 0) or is_metadata:
                ...
                shutil.copytree(os.path.join(source, channel, subdir),
    ...
    for archive_file in archive_files:
        ...
        ts = datetime.datetime.utcfromtimestamp(float(basename[basename.find('@')+1:-3]))
        if ts >= startDT and ts <= endDT:
            ...
            if not is_compressed and gzip > 0 and not is_metadata:
                ...
                cmd = 'h5repack -i %s -o %s -f rf_data:GZIP=%i' % (archive_file, src_file, gzip)
                try:
                ...
                shutil.copyfile(archive_file, os.path.join(target_dir, os.path.basename(archive_file)))
        elif ts > endDT:
            return
    ...
    if startDT < dt and nextDT != None:
        if nextDT < endDT:
            if (is_compressed or gzip == 0) or is_metadata:
                if verbose:
                    print('%s - complete copy' % (subdir))
                cmd = 'scp -q -r %s %s' % (os.path.join(source, channel, subdir),
                                        os.path.join(channel_dest, subdir))
                try:
                    subprocess.check_call(cmd.split())
                except subprocess.CalledProcessError:
                    raise IOError, 'Failed to copy entire subdirectory with cmd <%s>' % (cmd)
                return
...
class archive:
    ...
    """
    def __init__(self, startDT, endDT, source, dest, pool=1, channels=None, is_metadata=False,
                 gzip=1, verbose=False, check_only=False):
        """
        ...
        if self._dest_type == 'local' and self._source_type == 'local':
            ...
            if self.verbose or self.check_only:
                print('Estimated GB to use = %f GB, available = %f GB' % (needed_GB, avail_GB))
                if self.check_only:
                    sys.exit(-1)
            if needed_GB > avail_GB:
                raise IOError, "aborting because needed GB %f greater than available GB %f" % (needed_GB, avail_GB)
    ...
    def _archive_channel(self, channel):
        ...
        """
        # to simplify the code, break into for subroutines depending on self._source_type self._dest_type
        if self._source_type == 'local':
    ...
    def _archive_channel_local_local(self, channel):
        ...
        channel_dest = os.path.join(self.dest, self._top_level_dest_dir, self._next_level_dir,
                                    channel)
        ...
        for i, items in enumerate(subdir_list[0]):
            ...
            args_list.append([self.source, channel, subdir, dt, nextDT, self.is_metadata, channel_dest,
                    self.startDT, self.endDT, self.gzip, self.verbose])
            
        self._pool.map(archive_subdirectory_local_local, args_list)
    ...
    def _archive_channel_local_remote(self, channel):
        ...
        top_level_dest = os.path.join(self.dest, self._top_level_dest_dir, self._next_level_dir)
        channel_dest = os.path.join(self.dest, self._top_level_dest_dir, self._next_level_dir, channel)
        # make channel dir locally, then copy
        ...
        for metadata_file in subdir_list[1]:
            if self.verbose:
                print('archiving metadata file %s' % (metadata_file))
            cmd = 'scp -q %s %s' % (os.path.join(self.source, channel, metadata_file), channel_dest)
            try:
                subprocess.check_call(cmd.split())
            except subprocess.CalledProcessError:
                raise IOError, 'Failed to copy metadata file with cmd <%s>' % (cmd)
        ...
        self._pool.map(archive_subdirectory_local_remote, args_list)
    ...
    def _create_top_level_dest_dirs(self):
        ...
        if self._dest_type == 'local':
            ...
            # try to create the three remote directories - even if they already exist, should succeed
            for i, this_dir in enumerate((self._top_level_dest_dir, self._metadata_dir, self._rf_dir)):
                ...
                if i == 0:
                    ...
                    cmd = 'scp -q -r %s %s' % (localDir, os.path.join(self.dest, self._top_level_dest_dir))
                try:
                    subprocess.check_call(cmd.split())
                except subprocess.CalledProcessError:
                    raise IOError, 'Failed to create remote top level dir with cmd <%s>' % (cmd)
                try:
                    os.system('rm -rf %s' % (localDir))
                except:
                    pass
    ...
    def estimate_disk_space(self):
        ...
        for channel in self.channels:
            ...
            # metadata
            for metadata_file in subdir_list[1]:
                needed_GB += os.path.getsize(os.path.join(self.source, channel, metadata_file))
            ...
            for i, items in enumerate(subdir_list[0]):
                ...
                if self.is_metadata:
                    archive_files = glob.glob(os.path.join(self.source, channel, subdir, _metadata_file_glob))
                    compress = 1.0
                else:
                    archive_files = glob.glob(os.path.join(self.source, channel, subdir, _rf_file_glob))
    ...
    def _test_compression(self, testHdf5File):
        ...
        try:
            ...
            pass
        cmd = 'h5repack -i %s -o %s -f rf_data:GZIP=%i' % (testHdf5File, resultFile, self.gzip)
        subprocess.check_call(cmd.split())
    ...
    def get_chan_dict_local(self):
        ...
        """
        ret_dict = {}
        subdirectories = glob.glob(os.path.join(self.source, '*', self._sub_directory_glob))
        subdirectories.sort()
        ...
        metadata_files = glob.glob(os.path.join(self.source, '*', self._metadata_glob))
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='digital_rf_archive.py is a tool for archiving Digital RF data..')
    ...
    archive(startDT, endDT, args.source, args.dest, args.pool, args.channel, args.is_metadata,
            args.gzip, args.verbose, args.check_only)
